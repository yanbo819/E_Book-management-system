...
public class CheckOutServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        // This should ideally be a POST request as it changes the server state (creates orders).
        // For now, retaining doGet as per original code but strongly advise using doPost.
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (cart_list != null && auth != null && !cart_list.isEmpty()) { // Add check for empty cart
            try {
                ...
                if (allOrdersInserted) {
                    cart_list.clear(); // Clear cart only if all orders were successful
                    response.sendRedirect("orders.jsp?status=success");
                } else {
                    response.sendRedirect("orders.jsp?status=partial_failure"); // Indicate partial failure
                }
            ...
            } else { // cart_list is null or empty
